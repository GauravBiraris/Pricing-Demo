        ...
        if inventory:
            ...
            if not df.columns.isin(['Product ID']).any() or not df.columns.isin(['Cost Price']).any():
                ...
                if st.button("Get Prices"):
                    ...
                    dfl = df.merge(locked_prices, on='Product ID')

                    # Initialize output dataframe 
                    output = pd.DataFrame()
                    output['Product ID'] = dfl['Product ID']
                    output["Validity"]= dfl["Validity"]
                    for i, row in output.iterrows():
                        output.loc[i, 'Recommended Price'] = dfl.loc[i, 'Cost Price']+ max_margin*dfl.loc[i, 'Cost Price']/100
                    ...
                    output['Recommended Price'] = np.where(dfl["Locked Price"]>0, dfl["Locked Price"], output['Recommended Price'])                    
                    # output.loc[output['Locked Price'].isna(), 'Recommended Price'] = None

                    #Ensure None prices based on min. margin
                    for i, row in output.iterrows():
                        if output.loc[i, 'Recommended Price'] == None:
                            output.loc[i, 'Recommended Price'] = dfl.loc[i, 'Cost Price']+ min_margin*dfl.loc[i, 'Cost Price']/100
                            output.loc[i, 'Validity'] = 12
